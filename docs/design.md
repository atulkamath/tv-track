...
- **Show detail = centered modal** (max 640px, 84vh), not a drawer. Header: poster thumb, title, count, **"Mark all watched" / "Unmark all"** for the whole show, then ✕. Body scrolls; sticky season headers with per-season mark-all; per-episode checkboxes. Esc and outside-click close.
